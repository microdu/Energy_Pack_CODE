...
_iq VqTesting = _IQ(0.4);			// Vq reference (pu)
_iq IdRef = _IQ(0.0);				// Id reference (pu) 
_iq IqRef = _IQ(0.1);				// Iq reference (pu)


#if (BUILDLEVEL<LEVEL3)             // Speed reference (pu)
_iq  SpeedRef = _IQ(0.1);          // For Open Loop tests
#else
_iq  SpeedRef = _IQ(0.15);           // For Closed Loop tests
#endif
...
// Initialize ADC for DMC Kit Rev 1.1 Gizmo
	ChSel[0]=1;		// Dummy meas. avoid 1st sample issue Rev0 Picollo*/
	ChSel[1]=1;//9;//1;		// ChSelect: ADC A1-> Phase A Current
	ChSel[2]=11;//1;//9;		// ChSelect: ADC B1-> Phase B Current
	ChSel[3]=9;//3;		// ChSelect: ADC A3-> Phase C Current
...
// Initialize PGAs,Filter for DMC Kit Rev 1.1 Gizmo
	ANALOGSUBSYS_INIT_MACRO()

// TODO: PGA config here
	// ===========================================================
	EALLOW;
	AnalogSubsysRegs.AMPM1_GAIN.bit.AMPA1GAIN = 1;
	AnalogSubsysRegs.AMPM1_GAIN.bit.AMPA3GAIN = 1;
	AnalogSubsysRegs.AMPM1_GAIN.bit.AMPB1GAIN = 1;

	AnalogSubsysRegs.COMPEN.bit.COMPA1EN = 0;
	AnalogSubsysRegs.COMPEN.bit.COMPA3EN = 0;
	AnalogSubsysRegs.COMPEN.bit.COMPB1EN = 0;
	EDIS;
	// ===========================================================
...
//  Connect inputs of the PWM_DRV module and call the PWM signal generation macro
// ------------------------------------------------------------------------------
    pwm1.MfuncC1 = svgen1.Tb;//svgen1.Ta;
    pwm1.MfuncC2 = svgen1.Ta;//svgen1.Tb;
    pwm1.MfuncC3 = svgen1.Tc; 
	PWM_MACRO(1,2,3,pwm1)							// Calculate the new PWM compare values	
...
//    Connect inputs of the PWMDAC module 
// ------------------------------------------------------------------------------	
	pwmdac1.MfuncC1 = clarke1.As;//rg1.Out;//clarke1.Cs;//svgen1.Ta;
    pwmdac1.MfuncC2 = clarke1.Bs;//rg1.Out;//clarke1.Bs;//svgen1.Tb;
    PWMDAC_MACRO(6,pwmdac1)	  						// PWMDAC 6A, 6B
    
    pwmdac1.MfuncC1 = clarke1.Cs;;//rg1.Out;//clarke1.As;//svgen1.Tc;
    pwmdac1.MfuncC2 = rg1.Out;;//svgen1.Tb-svgen1.Tc;
...
//    Connect inputs of the PWMDAC module 
// ------------------------------------------------------------------------------	
	pwmdac1.MfuncC1 = clarke1.As;//rg1.Out;
    pwmdac1.MfuncC2 = clarke1.Bs;//speed1.ElecTheta;
    PWMDAC_MACRO(6,pwmdac1)	  						// PWMDAC 6A, 6B
    
    pwmdac1.MfuncC1 = clarke1.Cs;
...
//    Connect inputs of the DATALOG module 
// ------------------------------------------------------------------------------
    DlogCh1 = (int16)_IQtoIQ15(clarke1.Alpha);
    DlogCh2 = (int16)_IQtoIQ15(clarke1.Beta);
    DlogCh3 = (int16)_IQtoIQ15(volt1.Valpha);
    DlogCh4 = (int16)_IQtoIQ15(volt1.Vbeta); 
